...
SCRIPTS_DIR=${PROJECT_ROOT}/build-tools/scripts
...
make_pkg()
{
	...
	read version
	${SCRIPTS_DIR}/build-pkg.sh $version
}

if [ `dirname $0` != "./build-tools/scripts" ] && [ `dirname $0` != "build-tools" ]; then
        echo
